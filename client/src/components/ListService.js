...
function ListService(props) {
  ...
  const LuggageContent = <p>Hãy chọn gói hành lý </p>;

  const SeatMapAction = () => {
    return (
      <button
        onClick={() => setShowSeatModal(!showSeatModal)}
  ...
  const LuggageAction = () => {
    return (
      <button
        onClick={() => setShowLuggageModal(!showLuggageModal)}
        className="ui primary button"
      >
        OK
      </button>
  ...
  return (
    <div className="boxChoiceService">
      <div className="title">
        <span>
      ...
      <CardItem
        src="assets/select-service_favorite-seat.cc6498ae.svg"
